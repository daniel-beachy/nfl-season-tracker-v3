...
import { SOURCES, URLS, adaptFpi, fetchFutures, fetchLeaders, isolatedSource, loadFutures, fetchBovada, fetchBovadaAwards, fetchPolymarket, fetchPolymarketAwards } from './providers.mjs';
...
export async function capture({ season, scheduled = false, now = new Date(), root = ROOT, get = createHttpClient() } = {}) {
  ...
  const bovadaPayload = get(URLS.bovada);
  const polymarketPayload = get(URLS.polymarket);
  const [fpi, draftkings, bovada, polymarket, leaders, dkAwards, bovadaAwards, polymarketAwards] = await Promise.all([
    ...
    isolatedSource('Bovada', () => fetchBovada(season, teams, get, bovadaPayload)),
    isolatedSource('Polymarket', () => fetchPolymarket(season, teams, get, polymarketPayload)),
    fetchLeaders(season, metadata, teams, get),
    fetchAwards(season, teams, get, futures),
    fetchBovadaAwards(season, teams, get, bovadaPayload),
    fetchPolymarketAwards(season, teams, get, polymarketPayload),
  ]);
  for (const source of [fpi, draftkings, bovada, polymarket]) source.note = `${source.note} ${metadata.note}${teamWarning}`;
  const snapshot = {
    id: snapshotId(season, now), capturedAt: now.toISOString(), season,
    phase: metadata.phase, week: metadata.week,
    label: metadata.label,
    sources: { 'espn-fpi': fpi, draftkings, bovada, polymarket },
    leaders,
    awards: { draftkings: dkAwards, bovada: bovadaAwards, polymarket: polymarketAwards },
  };
  ...
  console.log(JSON.stringify({
    ...
    awards: Object.fromEntries(Object.entries(snapshot.awards).map(([id, award]) => [id, {
      status: award.status, categories: award.categories?.length ?? 0, note: award.note,
    }])),
  }, null, 2));
